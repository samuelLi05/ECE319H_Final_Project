...
void I2C_Init(void){
  ...
  // bits 4-0 I2C
  IOMUX->SECCFG.PINCM[PA18INDEX]  = 0x02040084;  // I2C SDA
  IOMUX->SECCFG.PINCM[PA15INDEX]  = 0x02040084;  // I2C SCL
...
// recieves a single byte from the specified slave
...
void I2C_ADXL345_Init(uint8_t slaveAddress)
{
  I2C_Init();
  I2C_Send2(slaveAddress, 0x2D, 0x08);  // Enable measurement mode
  I2C_Send2(slaveAddress, 0x31, 0x0B);  // Full resolution, +/-16g
  I2C_Send2(slaveAddress, 0x2C, 0x0A);
}

// Function to read multiple bytes from a register
int I2C_ReadRegister(uint8_t slave, uint8_t reg, uint8_t *buffer, uint16_t len) {
    if (I2C_Send1(slave, reg) == 0) return 0;  // Send register address
    while ((I2C1->MASTER.MSR & 0x20) == 0) {}; // Wait until idle
    
    I2C1->MASTER.MSA = (slave << 1) | 0x01;  // Read mode
    I2C1->MASTER.MCTR = (len << 16) | 0x07; // Read length with STOP

    for (int i = 0; i < len; i++) {
        while ((I2C1->MASTER.MFIFOSR & 0x000F) == 0) {}; // Wait for data
        buffer[i] = I2C1->MASTER.MRXDATA;
    }
    return 1;
}

// Optimized function to read accelerometer data
void adxl345_read_accel(int16_t *x, int16_t *y, int16_t *z) {
    uint8_t data[6];
    if (I2C_ReadRegister(0x53, 0x32, data, 6)) {
        *x = (int16_t)((data[1] << 8) | data[0]);
        *y = (int16_t)((data[3] << 8) | data[2]);
        *z = (int16_t)((data[5] << 8) | data[4]);
    } else {
        *x = *y = *z = 0; // Return 0 if read fails
    }
}

// void adxl345_read_accel(int16_t *x, int16_t *y, int16_t *z) {
//     uint8_t data[6];
//     data[0] = I2C_Recv1(0x32);
//     data[1] = I2C_Recv1(0x33);
//     data[2] = I2C_Recv1(0x34);
//     data[3] = I2C_Recv1(0x35);
//     data[4] = I2C_Recv1(0x36);
//     data[5] = I2C_Recv1(0x37);
